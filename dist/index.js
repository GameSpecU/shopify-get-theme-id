...
async function getAllThemes (SHOPIFY_AUTH){
  const {
    data: { themes },
  } = await lib_axios.get(
...
async function getThemeByName(SHOPIFY_AUTH, theme_name){
...
function getActionInputs() {
    const store_url = src_core.getInput('store', {required: true});
    const access_token = src_core.getInput('token', {required: true});
    const theme_name = src_core.getInput('theme_name', {required: true});
...
async function run() {
    try {
        ...
        src_core.setOutput('theme_id', theme.id);
